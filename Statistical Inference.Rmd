...
###### I denne oppgaven skal jeg se på variabelen **"T3T1_PERCENT_CHANGE_FAST_CSA"**. Denne variabelen beskriver endringen i type II muskelfibre fra pre-test til post-test etter at intervensjonsperioden var gjennomført. I løpet av denne oppgaven skal jeg prøve å svare på følgende spørsmål: Var det en signifikant endring i type II muskelfibre mellom de to gruppene i løpet av intervensjonsperioden? 

### Deskriptiv statistikk
###### Tabell 1 viser hvordan forskjellen mellom de to gruppene var ved pre-test. Jeg har valgt å ta med alder, vekt og mengden (i %) type II fibre forsøkpersonene hadde før intervensjonsperioden startet. Forsøkspersonene er gruppert i 2 grupper. HIGH-respondere og LOW-respondere. Tabellen representerer gjennomsnittsverdiene i de to gruppene.
...
df %>%
  mutate(variable = factor(variable, levels = c("T3T1_PERCENT_CHANGE_FAST_CSA", 
                           ...
                           labels = c("Type II fibre change (%)",
                                      "Type II fibre change (%)"))) %>% # Endrer variabelteksten.
...
###### Plottet under viser endringene i type II fibre i de to ulike gruppene. 
...
bxp <- ggboxplot(hyp, x = "CLUSTER", y = "T3T1_PERCENT_CHANGE_FAST_CSA", 
                 ylab = "Endringer i type II fiber (%)", xlab = "CLUSTER", add = "jitter", SIZE = 0.5) # Denne funskjonen lager et boxplot med utgangspunkt i variabelen hypertrophy.
...
### T-test
###### For å kunne svare på spørsmålet om det var en signifikant forskjell mellom endringene i de to gruppene må jeg gjennomføre en t-test. Før jeg gjennomfører t-testen ser jeg på om verdiene i gruppene er normalfordelt og om det er like stor varians i gruppene. 

###### Plottet under viser at gruppene er tilnærmet normalfordelt.
...
###### Deretter testet jeg variansen i mellom de to gruppene ved hjelp av en levene_test. Hvis p-verdien er mindre enn 0.05 er ikke variansen i gruppene lik. Er p-verdien over 0.05 er variansen i gruppene tilnærmet lik. Resultatet kan du lese av i koden under. 
...
###### Resultatet av variansen viste at variansen i gruppene var tilnærmet lik. Dette betyr at når t-testen gjennomfører må det registreres at variansen er lik. I og med at to forskjellige grupper skal testes opp mot hverandre må vi bruke en uavhengig t-test. Resultatet av t-testen ser du i koden under. Tabellen oppsummerer resultatene forenklet.
...
###### T-testen viste en signifikant P-verdi, men for å gjøre konklusjonen tydeligere er det greit å ta med effect size som et mål på hvor markant forskjellen faktisk er. Til å regne ut effect size bruker jeg Cohens D-formel. Koden under viser hvordan effect size regnes ut og resultatet.
